...
public class Conta implements Cadastro {
    ...
    public List<Movimentacao> getMovimentacoes() {
        if (movimentacoes == null)
            throw new NullPointerException("A lista de movimentações não pode ser nula");
        else
    ...
    public void setLimite(double limite) {
        if (!isEspecial() && limite > 0){
            throw new IllegalStateException("Somente contas especiais podem ter limite específico");
        }
